...
例如，对于自动驾驶汽车，在每个状态下都可以（几乎）无限地选择动作（将方向盘转动 15°、17.2°、19.4°、鸣喇叭等）。我们需要为每个可能的动作输出一个 Q 值！而采取连续输出的最大动作本身就是一个优化问题！
...
策略是**给定状态、输出动作分布** 的函数（在我们的例子中使用随机策略）。
