...
此外，自 2018 年以来，您现在可以访问如此多令人惊叹的环境和库来构建您的Agent。

这就是为什么这是开始学习的最佳时机，本门课程，您来对地方了。

因为本文是深度强化学习课程的第一个单元，这是一门从初学者到专家的免费课程，您将在其中学习使用著名的深度 RL 库（如 Stable Baselines3、RL Baselines3 Zoo 和 RLlib）的理论和实践。
...
- 环境进入新状态$S_1$——  游戏进入下一帧。
- 环境给予给Agent一些回报$R_1$——Agent没有死 （正面奖励+1）。
